package com.flink.tailnum.analysis

import java.util.{Date, Properties}

import com.flink.tailnum.utils.TimeHelper._
import com.flink.tailnum.utils.{InitPropertiesUtil, JedisClusterUtil, RegularExpressUtil, TimeHelper}
...
object TailNumVioDetails {
  ...
  def main(args: Array[String]): Unit = {
    ...
    //初始化kafka生产者
    val producer010 = new FlinkKafkaProducer(kafkaProp.getProperty("target2.topic"),
      new SimpleStringSchema, properties)

    //初始化kafka消费者
    val consumer010 = new FlinkKafkaConsumer(kafkaProp.getProperty("source.topic"),
      new SimpleStringSchema, properties)
